...
function imic_afterSavePost()
{
	...
	elseif($post_type=='page'){
		$post_opt = '';
		$section2_switch = get_post_meta($postId,'imic_status_section2',true);
		$section2_sidebar = get_post_meta($postId,'imic_section2_homepage_sidebar',true); 
				
				$recent_category_slug = '';
				$section2_heading = get_post_meta($postId,'imic_section2_heading',true);
				$recent_title = get_post_meta($postId,'imic_section2_recent_title',true);
				$status_recent_post = get_post_meta($postId,'imic_status_latest_post',true);
				$recent_posts_count = get_post_meta($postId,'imic_posts_to_show_on',true);
							$recent_posts_count = ($recent_posts_count=='')?3:$recent_posts_count;
							$recent_post_type = get_post_meta($postId,'imic_selected_post_type',true);
							if($recent_post_type=='post') {
							$recent_category = get_post_meta($postId,'imic_post_category',true);
							} elseif($recent_post_type=='event') {
							$recent_category = get_post_meta($postId,'imic_event_category',true);
							} elseif($recent_post_type=='gallery') {
							$recent_category = get_post_meta($postId,'imic_gallery_category',true);
							} elseif($recent_post_type=='project') {
							$recent_category = get_post_meta($postId,'imic_project_category',true);
							} elseif($recent_post_type=='product') {
							$recent_category = get_post_meta($postId,'imic_product_category',true);
							} else { $recent_category = ''; }
							$post_options = get_post_meta($postId,'imic_select_post_options',false);
							foreach($post_options as $option) {
								$post_opt .= $option.'|';
							}
							$post_content = get_post_meta($postId,'imic_select_post_content',true);
							$thumb_hyperlink = get_post_meta($postId,'imic_select_thumb_hyperlink',true);
							$title_hyperlink = get_post_meta($postId,'imic_select_title_hyperlink',true);
							...
							$recent_category= $recent_category_slug->slug; } }
							else {
							if(!empty($recent_category)){
							...
							$updated_value = "[multipost st=".$section2_switch." sb=".$section2_sidebar." ht=".$section2_heading." lps=".$status_recent_post." rpt=".$recent_title." posts=".$recent_post_type." category=".$recent_category." postopt=".$post_opt." content=".$post_content." imglink=".$thumb_hyperlink." titlelink=".$title_hyperlink." count=".$recent_posts_count."]";
							update_post_meta($postId,'section2_shortcode',$updated_value);
	 }
	}
}
imic_afterSavePost();
...
if (have_posts()):
    while (have_posts()):the_post();
        ...
        while ($fr_repeat <= $frequency_count) {
			...
			} } if($days_extra<1) { $fr_repeat++; } else { $fr_repeat = 1000000; }
        } 
    endwhile;
...
function imic_contact_event_manager(){
	...
	$e_body = "You have been contacted by $name, for $event_title." . PHP_EOL . PHP_EOL;
	$body = "Your message has been delivered to Event Manager for $event_title." . PHP_EOL . PHP_EOL;
$e_content = '';
$e_content .= "Name: $name $lname" . PHP_EOL . PHP_EOL;
$e_content .= "Email: $email" . PHP_EOL . PHP_EOL;
$e_content .= "Phone: $phone" . PHP_EOL . PHP_EOL;
$e_content .= "Notes: $notes" . PHP_EOL . PHP_EOL;
$e_reply = "You can contact $name via email, $email";
$reply = "You can contact manager via email, $manager_email";
...
$e_content = '';
$e_content .= "Registration Number: $event_registration" . PHP_EOL . PHP_EOL;
$e_content .= "Event: $event_title" . PHP_EOL . PHP_EOL;
$e_content .= "Event Date: $event_dt" . PHP_EOL . PHP_EOL;
$e_content .= "Name: $name $lname" . PHP_EOL . PHP_EOL;
$e_content .= "Email: $email" . PHP_EOL . PHP_EOL;
$e_content .= "Phone: $phone" . PHP_EOL . PHP_EOL;
...
function imic_get_event_details() {
	...
	$address2 = get_post_meta($id,'imic_event_address2',true);
  	$output .= '<span class="meta-data"><i class="fa fa-calendar"></i> <span class="event-date">'.esc_attr(date_i18n(get_option('date_format'),$key)).'</span>'.__(' at ','framework').'<span class="event-time">'.date_i18n(get_option('time_format'), $st_time).'</span></span>
                                    <span class="meta-data event-location-address"><i class="fa fa-map-marker"></i> '.$event_address.'</span>';
	$output .= '</div>';
