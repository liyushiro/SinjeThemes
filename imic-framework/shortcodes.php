<?php
	...
	function imic_staff($atts, $content = null) {
		...
		$url = imic_get_template_url('template-speakers-sermon.php');
			query_posts(array('post_type'=>'speaker','speaker-category'=>$cat,'posts_per_page'=>$number));
			$output .= '<h3>'.$title.'</h3>
              	<hr class="sm">';
			if($type==1) { 
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-7 col-sm-6">
                                        ...
                                        $post_id = get_post(get_the_ID());
											...
											if($url!='') {
										$output .= '<a class="btn btn-primary" href="'.esc_url(add_query_arg('speakers',get_the_ID(),$url)).'">'.__('View all Sermons','framework').'</a>'; } }
                                    $output .= '</div>
                                </div>
                            </div>
                            </div>
